...
function formatRemaining(ms: number, locale: string): string {
  ...
  const minStr = new Intl.NumberFormat(locale).format(min)
  const secStr = new Intl.NumberFormat(locale, { minimumIntegerDigits: 2 }).format(sec)
...
export function OtpExpiryCountdown({ sentAt, className }: OtpExpiryCountdownProps) {
  ...
  return (
    ...
    >
      ...
      {expired ? (
        <span>{t("reset.otp.expired")}</span>
      ) : (
        <span>
          {t("reset.otp.expiresIn")}{" "}
          <span className="font-mono font-semibold tabular-nums text-ink">
            {formatRemaining(remainingMs, i18n.language)}
          </span>
        </span>
      )}
    </div>
